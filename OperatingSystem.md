...
OSI的7层模型：物理层，数据链路层协议：arp（将网络层的ip地址解析为数据链路层的mac地址），网络层协议: ip，传输层协议：tcp、udp，会话层，表示层，应用层协议：http、dns、ftp 

Web服务器首次联网时，协议发生的顺序是arp->dns->http

ip协议是指在源地址和目的地址之间传送一种称之为数据报（包），不负责保证传送可靠性；ipv4有32位，ipv6有128位；ip地址分为5类，ABCDE，大型网络，中型网络，小型网络，多目地址，备用
*	A类网络：第一段是网络号0-127，最高位必须是0，子网掩码255.0.0.0，支持主机数256^3-2，私有IP为10.0.0.0 - 10.255.255.255即10.0.0.0/8
*	B类网络：前两段是网络号128-191，最高位必须是10，子网掩码255.255.0.0，支持主机数256^2-2，私有IP为172.16.0.0 - 172.31.255.255即172.16.0.0/12
*	C类网络：前三段是网络号192-223，最高位必须是110，子网掩码255.255.255.0，支持主机数256-2，私有IP为192.168.0.0 - 192.168.255.255即192.168.0.0/16
*	如果需要划分子网，则根据子网的数量算出对应二进制的位数，对应的子网掩码从网络号之后开始排列（例如C类网络划分3个子网，占用2位，对应子网掩码为255.255.255.192）
...
编译器的词法分析器是有限状态自动机，语法分析是下推（下推就是栈）自动机。下推自动机分为非确定性和确定性下推自动机，其中确定性是非确定的子集（非确定性下推自动机很难实现），通常用确定性下推自动机进行语法分析。语法分析阶段会生成parse tree

*	词法分析：识别源程序中的所有单词，分离出token
*	语法分析：识别由词法分析给出的单词符号序列是否是给定文法的正确句子
*	语义分析：词法分析和语法分析的基础上，确定标识符的类型并完成类型检查


解释器将源程序逐句翻译并立即执行，所以不产生目标程序，速度比编译器直接生产目标程序慢
